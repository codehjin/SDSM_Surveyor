...
/// <summary>포유류 탭의 세션 저장·복원.</summary>
public partial class MammalEntryViewModel : ITaxonSession
{
    string ITaxonSession.Key => TaxonKey;

    bool ITaxonSession.HasData => Entries.Any(e => !string.IsNullOrWhiteSpace(e.SpeciesKo));

    object ITaxonSession.CaptureState() => new MammalDraft { Rows = Entries.ToList() };

    void ITaxonSession.RestoreState(JsonElement json)
    {
        var d = json.Deserialize<MammalDraft>(SessionJson.Options);
        if (d is null) return;
...
/// <summary>포유류 세션 자료(관찰 행 목록). 구버전 임시저장 파일의 조사개황 항목은 무시된다.</summary>
public sealed class MammalDraft
{
    public List<MammalEntry> Rows { get; init; } = new();
}
