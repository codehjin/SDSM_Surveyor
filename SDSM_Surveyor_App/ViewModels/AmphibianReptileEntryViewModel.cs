...
public partial class AmphibianReptileEntryViewModel : ObservableObject, ITransientService
{
    ...
    private void AddQuick()
    {
        ...
        var entry = new AmphibianReptileEntry { SpeciesKo = QuickSpecies };
    ...
    /// 열 순서 = 그리드 순서 : 국명 · 학명 · 대분류 · 중분류 · 흔적6(성체·유생·알·울음소리·로드킬·기타)
    ...
    public void PasteRows(string clipboard)
    {
        ...
        foreach (var line in lines)
        {
            ...
            Entries.Add(new AmphibianReptileEntry
            {
                ...
                MajorCategory = Cell(cells, 2),
                MiddleCategory = Cell(cells, 3),
                Trace1 = Pi(Cell(cells, 4)),
                Trace2 = Pi(Cell(cells, 5)),
                Trace3 = Pi(Cell(cells, 6)),
                Trace4 = Pi(Cell(cells, 7)),
                Trace5 = Pi(Cell(cells, 8)),
                Trace6 = Pi(Cell(cells, 9)),
                Lat = Pd(Cell(cells, 10)),
                Lng = Pd(Cell(cells, 11)),
                Feature = Cell(cells, 12),
                Note = Cell(cells, 13),
            });
        }
    }
