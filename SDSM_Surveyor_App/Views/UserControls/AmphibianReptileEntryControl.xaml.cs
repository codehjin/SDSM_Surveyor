...
/// <summary>양서파충류 탭 입력 컨트롤. ViewModel은 DI로 주입.
/// 빠른 추가 바 포커스 이동과 그리드 Ctrl+V(엑셀 붙여넣기)만 코드비하인드에서 처리.</summary>
public partial class AmphibianReptileEntryControl : UserControl
{
    private readonly AmphibianReptileEntryViewModel _vm;

    public AmphibianReptileEntryControl()
    {
        InitializeComponent();
        _vm = App.Current.Services.GetRequiredService<AmphibianReptileEntryViewModel>();
        DataContext = _vm;
