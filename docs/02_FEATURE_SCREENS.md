...
| 분류군 | 순서 |
|---|---|
| 조류 | 국명 · 개체수 · 학명 · 도래유형 · 대항목 · 세부항목 · 서식유형 · 위도 · 경도 · 특징 · 특이사항 |
| 포유류 | 국명 · 학명 · 관찰지유형 · 흔적12(포획~기타) · 위도 · 경도 · 특징 · 특이사항 |
| 양서파충류 | 국명 · 학명 · 대분류 · 중분류 · 흔적6(성체~기타) · 위도 · 경도 · 특징 · 특이사항 |
> 조류는 어류와 같이 **[국명, 개체수] 2열만 붙여넣어도** 되도록 개체수를 국명 바로 옆에 두었다.
> 빠른추가 바의 개체수는 포유류=`관찰`(Trace2), 양서파충류=`성체`(Trace1)로 들어간다.

**⚠ 남은 제약 — 관찰형 3종의 학명 자동 채움 불가**
`species.json`의 `Bird`/`Mammal`/`Amphibian`은 **국명 문자열 목록**이라 종 선택 시 학명을 채울 근거가 없다.
현재는 학명을 **직접 입력·붙여넣기** 하도록 열어 두었다(필드 입력은 가능 = 완료 기준 충족).
자동 채움은 관리자 `species.json` 내보내기에 국명·학명 쌍이 포함되어야 가능 → `SDSM\docs\ADMIN_TASKS.md` §3에 기록.
