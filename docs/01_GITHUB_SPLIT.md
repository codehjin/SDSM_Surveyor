...
- [x] `SDSM_Surveyor`가 독립 저장소로 GitHub에 올라감
- [x] `SDSM`은 기존 주소 유지, 두 저장소의 원격 주소가 **서로 다름**
- [x] 두 저장소 모두 `bin/`·`obj/`가 추적되지 않음
- [x] 모든 md 문서가 각 저장소에 올라감
- [x] **작업 폴더의 파일이 하나도 삭제되지 않음** (특히 SQLite DB)

> 완료(2026-08-20): 조사자 `a0f5ea4` (79개 파일), 관리자 `799efb4`.
> ⚠ 조사자 `SDSM_Surveyor_App.csproj`는 `..\..\SDSM\SDSM_Models`를 ProjectReference로 참조한다.
> 저장소는 분리되었지만 **빌드하려면 두 저장소를 같은 상위 폴더에 나란히 clone**해야 한다.
